...
interface RestaurantsController {
     ...
     */
    @ApiOperation(value = "Lists the available restaurants for the current authenticated user. The list only "
            + "contains basic information like name and address and does not provide more complex fields "
            + "like: 'contractPlan' or 'featureFlags'.")
    @ApiResponses(value = [
        (ApiResponse(
                code = 200,
                message = "Success",
                response = Array<Restaurant>::class)),
        (ApiResponse(
                code = 401,
        ...
        (ApiResponse(
                code = 502,
                message = "If an error with an upstream external service was encountered.",
                response = Error::class))])
    @ApiImplicitParam(
            name = "authorization",
    ...
    @ApiResponses(value = [
        ...
        (ApiResponse(
                code = 404,
                message = "If the restaurant with the given id does not exist.",
                response = Error::class)),
        (ApiResponse(
                code = 500,
                message = "Internal Server Error",
